...
readMore.directive('readMore', function() {
    return {
        ...
        template:  "<p> {{text | readMoreFilter:[text, countingWords, textLength] }}" +
            "<a ng-show='showLinks' ng-click='changeLength()' class='color3' style='text-decoration:none'>" +
            "<strong ng-show='isExpanded' style='cursor:pointer;font-size:12px;'>  Show Less</strong>" +
            "<strong ng-show='!isExpanded' style='cursor:pointer;font-size:12px;'>  Show More</strong>" +
            "</a>" +
            "</p>",
